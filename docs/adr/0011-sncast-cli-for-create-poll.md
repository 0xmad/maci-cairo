...
JSON file passed as `--config` (`maci` plus schedule and poll public key; no
poll id). Stdout is the record (`maci`, `poll`, `poll_id` from `IPoll.poll_id`).
The caller is seed-0 `devnet-1`; we preflight coordinator. We rejected
