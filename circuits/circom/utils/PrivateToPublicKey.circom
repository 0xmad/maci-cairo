...
template PrivateToPublicKey() {
    ...
    signal isLessThan <== LessThan(251)([privateKey, SUBGROUP_ORDER]);
    isLessThan === 1;

    // Convert the private key to bits.
    signal computedPrivateBits[253] <== Num2Bits(253)(privateKey);

    // Perform scalar multiplication with the basepoint.
    signal computedPublicKey[2] <== EscalarMulFix(253, BASE8)(computedPrivateBits);
